...
class OLsmCategory
{
	...
	public function __construct($id, $type="id")
	{
		...
		else
		{
			...
			{
				$q = "SELECT * FROM lsm_category WHERE url_title = ?";
			}
	...
	{
		$CI =& get_instance();
		$CI->db->insert('lsm_category',$params);
	...
	public static function get_list($page=0, $limit=0, $orderby="id_lsm_category DESC", $not="")
	{
		$CI =& get_instance();
		...
		// not in
		if(trim($not) != "") $add_sql_stats .= " WHERE id_lsm_category NOT IN (' ".$not." ')";
		...
		// setup
		$res = $CI->db->query("SELECT SQL_CALC_FOUND_ROWS * FROM lsm_category {$add_sql_stats} ", $sql_arrs);
