	<?php
		...
		if($don->id_buyer != '')
		{
			$name = ($don->name != get_lsm($don->id_buyer)->name && $don->name != '')? $don->name : get_lsm($don->id_buyer)->name ;
			$email = ($don->email != get_lsm($don->id_buyer)->email && $don->email != '')? $don->email : get_lsm($don->id_buyer)->email ;
		} else {
			$name = $don->name;
			$email = $don->email;
		}
						...
						<div class="span9 thumbnail detail-boxes">
							...
							<?=form_open()?>
								<table>
									...
									<tr>
										<td colspan="3">
											<button type="button" class="btn btn-default" onclick="location.href='<?=base_url('organizer/donation_history')?>';">Back</button>
										</td>
									</tr>
								</table>
							<?=form_close()?>
