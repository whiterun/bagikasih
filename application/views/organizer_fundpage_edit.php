	<?php
		...
		$exp = ($fra->fund_target == 0.00) ? '' : str_replace('.00', '', $fra->fund_target) ;
	?>
	<body>
		<?=$this->load->view('head')?>
											...
											<tr>
												...
												<td>
													<div class="input-prepend">
														...
														<input type="text" name="target" value="<?=$exp?>" class="currfix" required />
													</div>
												</td>
											</tr>
											<tr>
												...
												<?php
													...
													foreach($fcl as $row) {
												?>
													<input type="radio" id="checkbox-2-<?=$i?>" class="regular-checkbox big-checkbox" name="pcolor" value="<?=$row->id_fcolor?>" <?=($fra->id_fcolor == $row->id_fcolor ? 'checked' : '' )?> /><label for="checkbox-2-<?=$i?>" style="background-color:<?=$row->color?>;"></label>
												<? $i++; } ?>
												</td>
											</tr>
											<tr>
												...
												<td>
													<?php foreach($l as $r) { ?>
														<div class="i-thumb">
															...
															<a href="<?=base_url("organizer/delete_fphoto/{$fra->id_fundraise}/{$r->id_fgallery}")?>" class="cl-fav btn btn-mini btn-danger" title="remove" onclick="return confirm('Delete this photo ?');"><i class="icon-remove"></i></a>
														</div>
													<? } ?>
												</td>
											</tr>
												...
												<td>
													<?php foreach($l as $r) { ?>
														<div class="input-append">
															...
															<a href="<?=base_url("organizer/delete_fvideo/{$fra->id_fundraise}/{$r->id_flvideo}")?>" class="btn btn-danger" title="remove" onclick="return confirm('Delete this photo ?');"><i class="icon-remove"></i></a>
														</div>
													<br>
													<? } ?>
												</td>
