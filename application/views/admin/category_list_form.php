				...
				<div class="block">
					...
					<div class="block-content collapse in">
						...
						<?php if(sizeof($list) <= 0) { ?>
							<p class='text_red'>Category List is empty.</p>
						<? } else { ?>
							<table class="table table-bordered table-striped tbl_list">
								...
								?>
									<tr>
										...
										<td><?=$row->category?></td>
										<td>
											<?=anchor($this->curpage.'/listing/'.$row->id_category, 'edit')?> |
											<?=anchor($this->curpage.'/delete/'.$row->id_category, 'delete', array("onclick" => "return confirm('Are you sure?');"))?>
										</td>
									</tr>
								<?php
									$i++;
